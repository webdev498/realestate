...
function getListingCount(){
  ...
  var n = [];
  for(i=0; i < neighborhoods.length; i++){
    if(neighborhoods[i].value != "All"){ n.push(neighborhoods[i].value); }
  }
  ...
  $('body').delegate('.clickGuestCompare','click',function(e){
    ...
    $.ajax({
      ...
      success: function(listings){
        ...
        // Automatically open the guest folder.
      ...
      success: function(listings){
        ...
        $.ajax({
          ...
          success: function(listings){
            ...
            // Automatically open the agent folder if they only have one.
            if (listings.length == 1){
              $(".toggleFolder").parents().eq(3).find(".listingSection").toggle(); // Open folder
              $(".toggleFolder").parents().eq(3).find(".closedFolder").hide(); // hide the closed folder icon
              $(".toggleFolder").parents().eq(3).find(".openFolder").show(); // show the open folder icon
                    ...
                    + "<tbody>";
                      folder.listings.forEach(function(listing){
                        html += "<tr>";
                          ...
                          + "<td style='text-align: center; width: 35px;'><img class='quality2 view-bubble-grades' style='cursor: pointer' src='"+listing.loc+"' height='16' data-toggle='tooltip' title='"+listing.locTitle+"'/></td>"
                          + "<td style='text-align: center; width: 39px;'><img class='quality2 view-bubble-grades' style='cursor: pointer' src='"+listing.bld+"' height='16' data-toggle='tooltip' title='"+listing.bldTitle+"'/></td>"
                          + "<td style='text-align: center; width: 37px;'><img class='quality2 view-bubble-grades' style='cursor: pointer' src='"+listing.vws+"' height='16' data-toggle='tooltip' title='"+listing.vwsTitle+"'/></td>"
                          + "<td style='text-align: center; width: 45px;'><img class='quality2 view-bubble-grades' style='cursor: pointer' src='"+listing.space+"' height='16' data-toggle='tooltip' title='"+listing.spaceTitle+"'/></td>"
...
$('.amenitiesmenu .amenity_icons').live('click',function(){
  ...
  if(amenities[amenity] === true){
    ...
    $('img.amenity-icons[rel='+amenity+']').attr('src','../images/amenities/'+amenity+'.png').removeClass('selected'); // change the icon color
    $('img.amenity_icons[rel='+amenity+']').attr('src','../images/amenities/'+amenity+'.png').removeClass('selected'); // change the icon color
    ...
    $('img.amenity-icons[rel='+amenity+']').attr('src','../images/amenities/'+amenity+'b.png').addClass('selected'); // change the icon color
    $('img.amenity_icons[rel='+amenity+']').attr('src','../images/amenities/'+amenity+'b.png').addClass('selected'); // change the icon color
    $.cookie(amenity, "true");
    setTimeout(function () { getListingCount(); }, 200);
  }
  return false;
