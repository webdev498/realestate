...
$(document).ready(function(){
  ...
  function getPopupHtml(clickedOn){
    return "<div class='need-to-signup-first-div bubble speech'>"
      +"<span class='fa fa-times need-to-signup-first-div-closer'></span>"
  ...
  function getSpaceFactorPopupHtml(){
    return "<div class='space-factor-popup bubble speech'>"
      +"<span class='fa fa-times space-factor-popup-closer'></span>"
  ...
  function getImplieSquareFootagePopupHtml(){
    return "<div class='implied-square-footage-popup bubble speech'>"
      +"<span class='fa fa-times implied-square-footage-popup-closer'></span>"
  ...
  function listingAlreadySaved(){
    return "<div class='need-to-signup-first-div bubble speech save-this-listing-popup' title='Save listing'>"
      +"<span class='fa fa-times need-to-signup-first-div-closer save-this-listing-popup-closer'></span>"
  ...
  $('body').delegate('.save-this-listing-popup-add-folder','click',function(e){
    ...
    if (typeof buyer != undefined && buyer != '') {
      $.ajax({
        type: "POST",
        url: "/controllers/dialog.php/",
        data: {"addNewFolder":"true", "email": buyer},
        async:false,
        success: function(folder){
          folder = JSON.parse(folder);
          $(e.target).parent('li').before(createNewFolderHtml(folder.name, index, folder.id));
          $('.save-this-listing-popup-add-folder').attr('data-index',++index);
        }
      });
    }
    else{
  ...
  function createNewFolderHtml(buyerName, index, folderId){
    var dateString = getFormattedDate();
    var html = "<li>"
        +"<input type='checkbox' class='save-this-listing-popup-checkbox' value='"+buyerName+"' />"
        +"<div data-index="+index+" class='save-this-listing-popup-new-folder-editable edit-div"+index+"'>"
        +"<span data-index='"+index+"' class='save-this-listing-popup-add-folder-surname surname"+index+" toHideOnEdit"+index+"'>"
        +buyerName
        +" </span>"
        +"<span data-index='"+index+"' class='toHideOnEdit"+index+"'>"
        +"(click to rename)"
        +"</span>"
        +"<span data-index='"+index+"' class='save-this-listing-popup-add-folder-date toHideOnEdit"+index+"'>"
        +"</span>"
        +"<div class='clear'>"
        +"</div>"
        +"</div>"
        +"<input type='text' style='display:none' value='"+buyerName+"' class='save-this-listing-popup-buyer-name-edit toShowOnEdit"+index+" surname-edit"+index+"'/>"
        +"<input type='hidden' style='display:none' value='"+folderId+"' class='folder-id-edit-surname-edit"+index+"'/>"
        +"<button style='display:none' data-index='"+index+"' class='updateBuyerName toShowOnEdit"+index+"'><i data-index='"+index+"' class='fa fa-check'></i></button>"
        +"<button style='display:none' data-index='"+index+"' class='toShowOnEdit"+index+" cancelUpdateName'><i data-index='"+index+"' class='fa fa-times'></i></button>"
      +"</li>";
    return html;
  }
  ...
  $('body').delegate('.save-this-listing-popup-new-folder-editable','click',function(e){
    var index = $(e.target).attr('data-index');
    toggleEdits(index);
  });

  $('body').delegate('.updateBuyerName','click',function(e){
    var index = $(e.target).attr('data-index');
    $(this).parent().find("input.save-this-listing-popup-checkbox").val($('.surname-edit'+index).val());
    $(".save-this-listing-popup-add-folder-surname.surname"+index).text($('.surname-edit'+index).val());
    toggleEdits(index);
  });

  $('body').delegate('.cancelUpdateName','click',function(e){
    var index = $(e.target).attr('data-index');
    toggleEdits(index);
  });

  function toggleEdits(index){
    $('.edit-div'+index).toggle();
    $('.toShowOnEdit'+index).toggle();
  }
  ...
  $('body').delegate('.save-this-listing-popup-footer button','click',function(e){
    ...
    if(!selected_folders.length){
      ...
      if(referrer == 'heart-list'){
        //var list_num = $(e.target).closest('tr').attr('id');
        var parent_p = $(e.target).closest('p#abc');
      } else if (referrer == 'heart-tab-head'){
        //var list_num = $(e.target).closest('div.tab-head-div').attr('data-list-num');
        var parent_p = $(e.target).closest('div.tab-head-div');
      } else if (referrer == 'heart-tab-inner'){
        //var list_num = $(e.target).closest('a.listing-small-icons').attr('data-list-num');
        var parent_p = $(e.target).closest('a.listing-small-icons');
      } else{
        $("#ajax-box").dialog({
          modal: true,
          height: 'auto',
  ...
  function getCompareListingsHTML(listings, user) {
      ...
      if (user != "agent-buyer"){
        html += "<span class='fa fa-times compare-listings-closer'></span>"
  ...
  function getAgentCompareListingsHTML(listings, user) {
    ...
    if (user == "agent") {
      html += "<div class='folder-section compare-saved-listings-div'>"
        + "<span class='fa fa-times compare-listings-closer'></span>"
        + "<div class='clearfix grpelem' id='u16159-5'>"
  ...
  function getFolderListingsHTML(listings, user) {
    ...
    if (listings.length > 0){
        ...
        html += "<div class='Text-1 clearfix u16157-235' style='margin-top: 0;'>"
          + "<div id='u16157-8'>"
            + "<div class='folderDetails' data-user='"+folder.user+"' data-folder='"+folder.name+"'>"
              + "<span id='u16157' class='toggleFolder' style='cursor: pointer;'><i class='fa fa-folder-o'></i></span>"
          ...
          + "<div class='listingSection'>"
            ...
            if(folder.listings.length > 0){
                  ...
                  + "<table id='fullTable' style='margin-left: 10px;'>"
                    ...
                    + "<tbody>";
                      folder.listings.forEach(function(listing){
                        html += "<tr>"
                          + "<td style='width: 173px;'><a style='cursor: 'pointer;'>"+listing.address+"</a></td>"
                          + "<td style='width: 60px;'>"+listing.apt+"</td>"
                          ...
                          + "<td style='text-align: center; width: 35px;'><img class='quality2' src='"+listing.loc+"' height='16'/></td>"
                          + "<td style='text-align: center; width: 39px;'><img class='quality2' src='"+listing.bld+"' height='16'/></td>"
                          + "<td style='text-align: center; width: 37px;'><img class='quality2' src='"+listing.vws+"' height='16'/></td>"
                          + "<td style='text-align: center; width: 45px;'><img class='quality2' src='"+listing.space+"' height='16'/></td>"
                          + "<td style='width: 33px;'></td>"
                          + "<td style='width: 290px;'>"+listing.comments+" &nbsp;<span id='u16157-13'><a class='add-edit-comment' style='cursor: pointer' data-user='"+folder.user+"' data-comment='"+listing.comments+"' data-listnum='"+listing.listing_num+"' data-folder='"+listing.folder+"' data-type='"+user+"'><i class='fa fa-plus'></i></a></span></td>"
                          + "<td style='width: 85 + 'px;'>"+listing.status+"</td>"
                          + "<td style='width: 80 + 'px;'>"+listing.date+"</td>"
                          + "<td>";
                            if (user == "guest") {
                              html += "<span id='u22086-58'><a><i class='fa fa-times deleteSavedGuestListing' data-folder='"+folder.name+"' data-listing='"+listing.listing_num+"'></i></a></span>";
                            }
                            else if (user == "buyer") {
                              html += "<span id='u22086-58'><a><i class='fa fa-times deleteSavedBuyerListing' data-folder='"+folder.name+"' data-listing='"+listing.listing_num+"'></i></a></span>";
                            }
                            else if (user == "agent") {
                              html += "<span id='u22086-58'><a><i class='fa fa-times deleteSavedAgentListing' data-folder='"+folder.name+"' data-listing='"+listing.listing_num+"'></i></a></span>";
                            }
                            else if (user == "agent-buyer") {
                              html += "<span id='u22086-58'><a><i class='fa fa-times agentDeleteSavedBuyerListing' data-buyer='"+folder.user+"' data-folder='"+listing.folder+"' data-listing='"+listing.listing_num+"'></i></a></span>";
                            }
                          html += "</td>"
                        + "</tr>";
                      });
              ...
              html += "<div style='width: 250px; min-height: 200px; margin-left: 40px; font-size: 16pt;'><span>No Saved Listings</span></div>"
            }
          html += "</div>"
        + "</div>";
      });
      ...
      html += "<div>"
        + "<div id='noFolders' style='width: 250px; min-height: 200px; margin-left: 10px; font-size: 16pt: display: none;'><span>No Saved Folders</span></div>"
      + "</div>"
    }
    return html;
  }
  ...
  function getEmailFolderHTML(user, folder, type) {
    ...
    html += "<div id='email-folder-popup'>"
      + "<div class='text-popups clearfix grpelem closeEmailFolderPopup' id='closeEmailFolderPopup'>"
        + "<h4 style='cursor: pointer;'><i class='fa fa-times'></i></h4>"
      + "</div>"
      + "<h2 class='Subhead-2' id='u1330-2'>Email Folder</h2>"
      + "<h4 class='text-popups' id='u1330-3'>&nbsp;</h4>"
      + "<h4 class='text-popups padBottom' id='u1330-5'>Send folder to:</h4>"
      + "<h4 class='text-popups' id='u1330-10'><span id='u1330-7'><input type='text' id='recipient' name='recipient' placeholder='Enter address'/></span></h4>"
  ...
  function getAddEditCommentHTML(email, folder, listing, comment, type) {
    ...
    html += "<div id='add-comment-form' title='Add Comment'>"
      + "<i id='closePopup2' class='fa fa-times close-add-edit-comment-popup'></i>"
...
var priceSort = { // Used to toggle price order in search results by ascending or descending
  order : 'asc',
  toggle : function(){
    if (priceSort.order === 'asc'){ priceSort.order = 'desc'; }
    else { priceSort.order = 'asc'; }
  }
};
  ...
  /*
  $('#jqgh_amen').live('hover',function(){ $('.amenitiesmenu').show(); });
  $('.amenitiesmenu:not(.criterion)').live('mouseleave',function(){ $('.amenitiesmenu').hide(); });
  $('.table-header').live('mouseleave',function(){ $('.amenitiesmenu').hide(); });
  $('#jqgh_price').live('hover',function(){ $('.amenitiesmenu').hide(); });
  $('#jqgh_loc').live('hover',function(){ $('.amenitiesmenu').hide(); });
  $('#jqgh_bld').live('hover',function(){ $('.amenitiesmenu').hide(); });
  $('#jqgh_vws').live('hover',function(){ $('.amenitiesmenu').hide(); });
  $('#jqgh_vroom_sqf').live('hover',function(){ $('.amenitiesmenu').hide(); });

  $('#jqgh_amen').live("click",function(){
    e.preventDefault();
    $('.amenitiesmenu').toggle();
    $('.south-arrow').toggle();

    return false;
  });
  $("#closeMenu").live("click",function(){ $('.amenitiesmenu').toggle(); });
  */
    ...
    if (!$(this).parents('.amenitiesmenu').hasClass("criterion")) {
      ...
      if (url.indexOf('/search.php') > -1){
        ...
        if($('#list').jqGrid('getGridParam', 'reccount') == 0){
          ...
          $('#jqgh_address').html('<span style="color: #AAAAAA;font-size: 0.8em;font-weight: normal;left: -9.8em;position: relative;text-align: left;top: 0.05em;">0 listings</span>');
          $('#listingsNumbersNearToSearch').html('<span>0 listings</span>');
        }else{
          $("#noListings").hide();
          $("#gview_list").show();
        }
...
$(function() {
  ...
  $("#redirecting").live('click', function(){
    ...
    $.get("/controllers/ajax.php", {
      ...
      value: "false",
      success: function(result){
        window.location = "http://homepik.com";
      }
    });
  });
  ...
  $("#bedroom_width,#bedroom_length,#living_width,#living_length").change(function(){
    ...
    if (bedroom_area == 0){ bedroom_area = (bedroom_length + bedroom_width); }
    if (living_area == 0){ living_area = (living_length + living_width); }
    if (bedroom_area > 0){ $("#bedroom_area").text(bedroom_area); }
    if (living_area > 0){ $("#living_area").text(living_area); }
  });

  $('ul.ui-tabs-nav a').live('click',function(){
  ...
  /* $('.ui-tabs-nav a').live('click',function(){
     alert("You've been clicked!");
  }); */
