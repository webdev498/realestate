...
if($limit != 'clear') { limit(); }
  ...
  elseif(isset($_SESSION['buyer'])){
	...
	$tplvar['name'] = $name;
	$tplvar['buyers'] = isset($buyers) ? $buyers : "";
	$tplvar['role'] = $_SESSION['role'];
	$tplvar['email'] = $_SESSION['email'];
	$tplvar['guestID'] = isset($_SESSION['guestID']) ? $_SESSION['guestID'] : "";
  $tplvar['agentID'] = $agentID;
  $tplvar['numSearches'] = $num_searches;
  $tplvar['justRegSaveForm'] = isset($justReg) ? $justReg : "";
	$tplvar['messages'] = isset($_SESSION['unreadMessages']) ? $_SESSION['unreadMessages'] : "";
	...
	$templatebody = $smarty->fetch('search-mobile.tpl.php', array('tplvar' => $tplvar));
	// Display the templated search page
	print $templatehead;
  include_javascript("/js/search-react-mobile.js", "text/babel");
  include_javascript("/js/search-mobile.js");
  include_css("/views/css/buying-formula-popup.css");
  include_css("/views/css/master_c-menu---search-field.css");
  include_css("/views/css/search-mobile.css");
	print $templatebody;
};
?>
