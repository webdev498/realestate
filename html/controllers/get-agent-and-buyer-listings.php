...
if($email != ''){
  ...
  foreach ($folders as $folder) {
    ...
    if ($last_update != 0 && empty($folder['last_update'])) { $folder['last_update'] = $last_update; }

    $results[] = $folder;
  }
  
  $result = mysql_query( "SELECT agent_id FROM `registered_agents` where (email = '" . $email . "')" ) or die("Couldn't execute query." . mysql_error());
  $row = mysql_fetch_array($result, MYSQL_ASSOC);
  $agent_id = $row['agent_id'];
  
  $day = date('U') - 86400;
  $result = mysql_query( "SELECT * FROM users_folders WHERE (agent LIKE '%".$agent_id."%') AND (last_update > ".$day.") ORDER BY last_update DESC" ) or die("Couldn't execute query.".mysql_error());
