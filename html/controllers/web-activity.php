...
include_once("head-reduced.php");
if(!isset($_SESSION['agent'])){ header('location: /users/logout.php'); }
else{
...
<head>
	...
	<style>
	  body { font-family: 'Lato', sans-serif; }
		.navbar-default{ border-left-style: none; border-right-style: none; background-color: white; }
		#wrapper{ max-width: 960px; margin: auto; }
		#header{ max-width: 960px; margin: auto; }
		#head-content span{ font-size: 0.6em; margin-bottom: 10px; color: #0098D4; }
		#logo { position: absolute; top: 1px; left: 10px; height: 70px; margin: 5px 4px; opacity: 0.8; }
	  .logout{ right: 0; }  
		.axis path, .axis line{ fill: none; stroke: #777; shape-rendering: crispEdges; }
		.axis text{ font-family: 'Arial'; font-size: 13px; }
