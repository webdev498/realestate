...
if( isset($_GET['getMyFolders']) ){
  ...
  $foldernames = array();
  foreach ($folders as $folder){
    $result = mysql_query( "SELECT name FROM `users_folders` where (id = '" . $folder . "')" ) or die("Couldn't execute query." . mysql_error());
    while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
      $foldername = $row['name'];
    }
    array_push($foldernames, $foldername);
		...
		if($_SESSION['agent']){
		  $res = mysql_query( "INSERT INTO queued_listings(`user`,`list_num`,`saved_by`,`comments`, `folder`, `role`, `time`) VALUES  ('".$_SESSION['email']."','".$list_num."','".$_SESSION['email']."','','".$foldername."','".$role."','".date('U')."')" ) or die(mysql_error());
		  $res2 = mysql_query( "UPDATE `users_folders` SET `last_update`='".date('U')."' WHERE (`user` = '".$_SESSION['email']."') AND (`name` = '".$foldername."')" ) or die(mysql_error());
		}
		else{
		  $res = mysql_query( "INSERT INTO saved_listings(`user`, `folder`, `time`, `saved_by`, `comments`, `role`, `list_num`) VALUES  ('".$_SESSION['email']."', '".$foldername."', '".date('U')."', '".$_SESSION['email']."', '', '".$role."', '".$list_num."')" ) or die(mysql_error());
		  $res2 = mysql_query( "UPDATE `users_folders` SET `last_update`='".date('U')."' WHERE (`user` = '".$_SESSION['email']."') AND (`name` = '".$foldername."')" ) or die(mysql_error());
    }
  }
  ...
  foreach ($folders as $folder) {
    ...
    if ($num < 1){
      $res = mysql_query( "INSERT INTO saved_listings(`user`,`list_num`,`saved_by`,`comments`, `role`, `folder`, `time`) VALUES  ('".$buyer."','".$list_num."','".$_SESSION['email']."','','buyer','".$folder."','".$time."')" ) or die(mysql_error());
      $res2 = mysql_query( "UPDATE	users_folders SET last_update='".$time."' WHERE (user ='". $buyer ."') AND (name='". $folder ."')" ) or die(mysql_error());
      
      $result = mysql_query( "SELECT agent FROM `users_folders` WHERE (user = '".$buyer."') AND (name = '".$folder."')" ) or die("Couldn't execute query.".mysql_error());
      $row = mysql_fetch_array($result,MYSQL_ASSOC);
      $agent_id = $row['agent'];
      
      $result = mysql_query( "SELECT first_name, last_name, email FROM `registered_agents` WHERE (agent_id = '".$agent_id."')" ) or die("Couldn't execute query.".mysql_error());
      $row = mysql_fetch_array($result,MYSQL_ASSOC);
      ...
      $message = "Hello " . $agent_firstname . " " . $agent_lastname . ",";
      $message .= "<br><br>" . $_SESSION['firstname'] . " " . $_SESSION['lastname'] . " has saved a new listing to your folder: " . $folder;
      //$message .= "<br><br>Listing Link: http://homepik.com/controllers/single-listing.php?list_numb=". $list_num;
      $message .= "<br><br><br><br>&copy; Nice Idea Media  All Rights Reserved<br>";
      $message .= "HomePik.com is licensed by Nice Idea Media";
  ...
  foreach ($buyers as $buyer) {    
    if(strpos($buyer, "@bellmarc.com") !== false){
      ...
      if ($num < 1){ $res = mysql_query( "INSERT INTO queued_listings(`user`,`list_num`,`saved_by`,`comments`, `role`, `folder`, `time`) VALUES  ('".$buyer."','".$list_num."','".$_SESSION['email']."','','agent','Agent Folder','".$time."')" ) or die(mysql_error()); }
  ...
  $res = mysql_query( "INSERT INTO saved_listings(`user`,`list_num`,`saved_by`,`comments`, `role`, `folder`, `time`) VALUES  ('".$_SESSION['guestID']."','".$list_num."','".$_SESSION['guestID']."','NULL','guest','Guest','".date('U')."')" ) or die(mysql_error());
  if($res){ print 1; }
  else{ print 0; }
}
else if ( isset($_GET['createGuestFolder'])){
	$res = mysql_query( "SELECT * FROM users_folders WHERE `user` = '".$_SESSION['guestID']."'" ) or die(mysql_error());
	$num = mysql_num_rows($result);
	if($num > 0){ print 1; }
  else{
    $res = mysql_query( "INSERT INTO users_folders(`user`,`name`, `agent`, `last_update`) VALUES  ('".$_SESSION['guestID']."','Guest','NULL','".date('U')."')" ) or die(mysql_error());
    if($res){ print 1; }
    else{ print 0; }      
	}    	
}
